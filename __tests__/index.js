...
describe('functional tests', () => {
    ...
    it('should set styles correctly on the root', () => {
        render(
            <CountdownCircleTimer {...fixture} size={240} />
        );
    ...
    it('should set aria-label attribute', () => {
        render(
            <CountdownCircleTimer {...fixture} ariaLabel="Nedtællingsur" />
        );
    ...
    it('should add linearGradient tag with the ID provided when isLinearGradient is true', () => {
        const { container } = render(
            <CountdownCircleTimer {...fixture} isLinearGradient gradientUniqueKey="test" />
        );

        const linearGradient = container.querySelector('linearGradient');
        expect(linearGradient).toBeInTheDocument();
        expect(linearGradient).toHaveAttribute('id', 'countdown-circle-timer-gradient-test');
    });
    ...
    it('should set stroke as the gradient Id on the path that animates if isLinearGradient is true', () => {  
        ...
        const path = container.querySelectorAll('path')[1];
        expect(path).toHaveAttribute('stroke', 'url(#countdown-circle-timer-gradient-test)');
    });
    ...
    it('should render the time', () => {  
        ...
        render(
            <CountdownCircleTimer {...fixture} renderTime={renderTime} />
        );
    ...
    it('should pass isPlaying and config options to useElapsedTime', () => {  
        ...
        render(
            <CountdownCircleTimer {...fixture} startAt={startAt} isPlaying={isPlaying} />
        );
...
describe('behaviour tests', () => {
    it('should fire onComplete', () => {  
        ...
        render(
            <CountdownCircleTimer {...fixture} onComplete={onComplete} />
        );
